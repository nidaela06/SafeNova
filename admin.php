<?php
// SafeNova Admin Paneli - Professional Redesign
session_start();
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .bg-canvas::after {
    ...
    background: radial-gradient(circle, rgba(155,29,255,0.15) 0%, transparent 70%);
  ...
  .brand-shield {
    ...
    box-shadow: 0 8px 24px rgba(232,49,90,0.3);
  }
  .brand-name {
  ...
  .field input:focus {
    border-color: rgba(232,49,90,0.5);
    box-shadow: 0 0 0 3px rgba(232,49,90,0.1);
  }
  ...
  <div class="login-card">
    ...
    <form method="POST">
      ...
      <div class="field">
        ...
        <input type="email" name="email" placeholder="admin@safenova.com" required>
      </div>
      <div class="field">
        <label>Şifre</label>
...
if (isset($_GET['delete_report']) && is_numeric($_GET['delete_report'])) {
    ...
    $stmt->bind_param("i", $report_id); $stmt->execute(); $stmt->close();
...
if (isset($_GET['delete_photo']) && is_numeric($_GET['delete_photo'])) {
    ...
    if ($photo) { $fp=$photo['file_path']; if(file_exists($fp))unlink($fp); if(file_exists(__DIR__.'/'.$fp))unlink(__DIR__.'/'.$fp); }
...
if (isset($_GET['delete_audio']) && is_numeric($_GET['delete_audio'])) {
    ...
    if ($audio) { $fp=$audio['file_path']; if(file_exists($fp))unlink($fp); if(file_exists(__DIR__.'/'.$fp))unlink(__DIR__.'/'.$fp); }
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    :root {
      --brand:    #e8315a;
      --brand-2:  #9b1dff;
      --brand-glow: rgba(232,49,90,0.25);
      --dark:     #08080d;
      --surface:  #101017;
      --surface-2:#16161f;
      --surface-3:#1d1d28;
    ...
    .user-avatar {
      ...
      background: linear-gradient(135deg, var(--brand), var(--brand-2));
      display: flex; align-items: center; justify-content: center;
      font-size: 14px; color: white; font-family: 'Syne', sans-serif;
    ...
    .stat-users   { --stat-color: #818cf8; }
    .stat-contacts{ --stat-color: #34d399; }
    .stat-reports { --stat-color: #f87171; }
    .stat-photos  { --stat-color: #60a5fa; }
    .stat-audios  { --stat-color: #a78bfa; }

    .si-users    { background: rgba(129,140,248,0.15); color: #818cf8; }
    .si-contacts { background: rgba(52,211,153,0.15); color: #34d399; }
    .si-reports  { background: rgba(248,113,113,0.15); color: #f87171; }
    .si-photos   { background: rgba(96,165,250,0.15); color: #60a5fa; }
    .si-audios   { background: rgba(167,139,250,0.15); color: #a78bfa; }
    ...
    .photo-meta .btn-delete { width: 100%; justify-content: center; }
    ...
    audio::-webkit-media-controls-panel { background: var(--surface-3); border-radius: 8px; }
...
<div class="layout">
  ...
  <aside class="sidebar">
    ...
    <div class="sidebar-foot">
      <div class="sidebar-user">
        ...
        <a href="admin.php?action=logout" class="logout-link" title="Çıkış Yap">
  ...
  <div class="main">
    ...
    <div class="content">
      ...
      ?>
        ...
        <div class="stats-row">
          <div class="stat-card stat-users">
            <div class="stat-row">
              <div class="stat-icon-wrap si-users"><i class="fas fa-users"></i></div>
            </div>
            <div class="stat-num"><?= $toplam_users ?></div>
            <div class="stat-label">Toplam Kullanıcı</div>
          </div>
          <div class="stat-card stat-contacts">
            <div class="stat-row">
              <div class="stat-icon-wrap si-contacts"><i class="fas fa-address-book"></i></div>
            </div>
            <div class="stat-num"><?= $toplam_contacts ?></div>
            <div class="stat-label">Acil Kişi</div>
          </div>
          <div class="stat-card stat-reports">
            <div class="stat-row">
              <div class="stat-icon-wrap si-reports"><i class="fas fa-triangle-exclamation"></i></div>
            </div>
            <div class="stat-num"><?= $toplam_reports ?></div>
            <div class="stat-label">İhbar Kaydı</div>
          </div>
          <div class="stat-card stat-photos">
            <div class="stat-row">
              <div class="stat-icon-wrap si-photos"><i class="fas fa-camera"></i></div>
            </div>
            <div class="stat-num"><?= $toplam_photos ?></div>
            <div class="stat-label">Fotoğraf</div>
          </div>
          <div class="stat-card stat-audios">
            <div class="stat-row">
              <div class="stat-icon-wrap si-audios"><i class="fas fa-microphone"></i></div>
            </div>
            <div class="stat-num"><?= $toplam_audios ?></div>
            <div class="stat-label">Ses Kaydı</div>
          </div>
        </div>
        ...
        <div class="dash-grid">
          ...
          <div class="card">
            ...
            <div class="table-wrap">
              <table class="data-table">
                <thead>
                  <tr><th>Ad Soyad</th><th>E-posta</th><th>Tarih</th></tr>
                </thead>
                <tbody>
                <?php
                $has = false;
                while($u=$son_users->fetch_assoc()): $has=true; ?>
                  <tr>
                    <td class="cell-name"><?= htmlspecialchars($u['ad_soyad']) ?></td>
                    <td class="cell-email"><?= htmlspecialchars($u['email']) ?></td>
                    <td class="cell-date"><?= date('d.m.Y H:i',strtotime($u['created_at'])) ?></td>
                  </tr>
                <?php endwhile;
                if(!$has): ?><tr><td colspan="3"><div class="empty-state"><i class="fas fa-inbox"></i><p>Kayıt yok.</p></div></td></tr><?php endif; ?>
                </tbody>
              </table>
            </div>
          </div>
          ...
          <div class="card">
            ...
            <div class="table-wrap">
              <table class="data-table">
                <thead>
                  <tr><th>Kullanıcı</th><th>Tarih</th></tr>
                </thead>
                <tbody>
                <?php
                $has2=false;
                while($r=$son_reports->fetch_assoc()): $has2=true; ?>
                  <tr>
                    <td class="cell-name"><?= htmlspecialchars($r['ad_soyad']) ?></td>
                    <td class="cell-date"><?= date('d.m.Y H:i',strtotime($r['created_at'])) ?></td>
                  </tr>
                <?php endwhile;
                if(!$has2): ?><tr><td colspan="2"><div class="empty-state"><i class="fas fa-inbox"></i><p>İhbar yok.</p></div></td></tr><?php endif; ?>
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div class="card">
          ...
          <div class="table-wrap">
            <table class="data-table">
              <thead>
                <tr><th>Tür</th><th>Kullanıcı</th><th>Dosya</th><th>Tarih</th></tr>
              </thead>
              <tbody>
              <?php
              $has3=false;
              while($m=$son_medya->fetch_assoc()): $has3=true; ?>
                <tr>
                  <td>
                    <?php if($m['tip']=='foto'): ?>
                      <span style="background:rgba(96,165,250,0.1);color:#60a5fa;padding:3px 10px;border-radius:20px;font-size:12px;"><i class="fas fa-camera" style="margin-right:4px;"></i>Fotoğraf</span>
                    <?php else: ?>
                      <span style="background:rgba(167,139,250,0.1);color:#a78bfa;padding:3px 10px;border-radius:20px;font-size:12px;"><i class="fas fa-microphone" style="margin-right:4px;"></i>Ses</span>
                    <?php endif; ?>
                  </td>
              ...
              <?php endwhile;
              if(!$has3): ?><tr><td colspan="4"><div class="empty-state"><i class="fas fa-inbox"></i><p>Medya yok.</p></div></td></tr><?php endif; ?>
              </tbody>
            </table>
          </div>
        </div>
      ...
      ?>
          ...
          <div style="background:var(--surface);border:1px solid var(--border-2);border-radius:20px;width:560px;max-width:95vw;max-height:90vh;overflow-y:auto;box-shadow:0 40px 80px rgba(0,0,0,0.5);">
            <div style="padding:20px 24px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;">
              <div style="font-family:'Syne',sans-serif;font-weight:700;font-size:16px;display:flex;align-items:center;gap:8px;">
                ...
                <span id="modal-name">Kullanıcı Detayı</span>
              </div>
            ...
            <div style="padding:24px;">
              ...
              <!-- Sağlık Bilgileri -->
              <div>
                <div style="font-size:11px;letter-spacing:1px;text-transform:uppercase;color:var(--muted);font-weight:600;margin-bottom:12px;display:flex;align-items:center;gap:6px;">
                  <i class="fas fa-heart-pulse" style="color:#f87171;"></i> Sağlık Bilgileri
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                  <div class="detail-box detail-health">
                    <span class="detail-label"><i class="fas fa-droplet" style="color:#f87171;margin-right:4px;"></i>Kan Grubu</span>
                    <span class="detail-val" id="m-kan" style="font-size:20px;font-family:'Syne',sans-serif;font-weight:800;color:#f87171;">—</span>
                  </div>
                  <div class="detail-box detail-health">
                    <span class="detail-label"><i class="fas fa-stethoscope" style="color:#60a5fa;margin-right:4px;"></i>Kronik Hastalık</span>
                    <span class="detail-val" id="m-kronik">—</span>
                  </div>
                  <div class="detail-box detail-health">
                    <span class="detail-label"><i class="fas fa-pills" style="color:#a78bfa;margin-right:4px;"></i>İlaç Kullanımı</span>
                    <span class="detail-val" id="m-ilac">—</span>
                  </div>
                  <div class="detail-box detail-health">
                    <span class="detail-label"><i class="fas fa-triangle-exclamation" style="color:#f59e0b;margin-right:4px;"></i>Alerji</span>
                    <span class="detail-val" id="m-alerji">—</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <style>
          ...
          .btn-detail {
            ...
            color:#818cf8;padding:7px 12px;border-radius:var(--radius-sm);
        ...
        <div class="section-header">
          ...
          <p>Toplam <?= count($users) ?> kullanıcı kayıtlı. Detay butonuna tıklayarak sağlık bilgilerini görüntüleyin.</p>
        </div>
        <div class="card">
          <div class="card-head">
            <div class="card-title"><i class="fas fa-users"></i> Tüm Kullanıcılar</div>
            <span class="card-count"><?= count($users) ?> kayıt</span>
          </div>
          <div class="table-wrap">
            <table class="data-table">
              <thead><tr><th>#</th><th>Ad Soyad</th><th>E-posta</th><th>Kan Grubu</th><th>Kayıt Tarihi</th><th>İşlem</th></tr></thead>
              <tbody>
              <?php if (count($users)==0): ?>
                <tr><td colspan="6"><div class="empty-state"><i class="fas fa-users-slash"></i><p>Kayıtlı kullanıcı bulunmuyor.</p></div></td></tr>
              <?php else: foreach ($users as $u):
                $kan = htmlspecialchars($u['kan_grubu'] ?? '');
                $kanBadgeColor = $kan ? '#f87171' : 'var(--muted)';
              ?>
                <tr>
                  ...
                  <td>
                    <?php if ($kan): ?>
                      <span style="background:rgba(248,113,113,0.15);color:#f87171;padding:3px 10px;border-radius:20px;font-size:13px;font-weight:700;font-family:'Syne',sans-serif;">
                        <i class="fas fa-droplet" style="font-size:10px;margin-right:3px;"></i><?= $kan ?>
                      </span>
                    <?php else: ?>
                  ...
                  <td>
                    <div class="action-group">
                      <button class="btn-detail" onclick="openModal(<?= htmlspecialchars(json_encode($u), ENT_QUOTES) ?>)">
                        <i class="fas fa-eye"></i> Detay
                      </button>
                      <a href="admin.php?action=users&delete_user=<?= $u['id'] ?>" class="btn-delete"
                         onclick="return confirm('<?= addslashes(htmlspecialchars($u['ad_soyad'])) ?> adlı kullanıcıyı silmek istediğine emin misin?')">
      ...
      ?>
        <div class="section-header">
          <h1>Acil Durum Kişileri</h1>
          <p>Toplam <?= count($contacts) ?> acil kişi tanımlı.</p>
        </div>
        <div class="card">
          <div class="card-head">
            <div class="card-title"><i class="fas fa-address-book"></i> Acil Kişiler</div>
            <span class="card-count"><?= count($contacts) ?> kayıt</span>
          </div>
          <div class="table-wrap">
            <table class="data-table">
              <thead><tr><th>#</th><th>Kişi Adı</th><th>Telefon</th><th>Yakınlık</th><th>Kullanıcı</th><th>Tarih</th><th>İşlem</th></tr></thead>
              <tbody>
              <?php if (count($contacts)==0): ?>
                <tr><td colspan="7"><div class="empty-state"><i class="fas fa-user-slash"></i><p>Acil kişi bulunmuyor.</p></div></td></tr>
              <?php else: foreach ($contacts as $c): ?>
                <tr>
                  ...
                  <td>
                    <span style="background:var(--surface-3);padding:3px 9px;border-radius:20px;font-size:12px;">
                      <?= htmlspecialchars($c['yakinlik'] ?? '—') ?>
                    </span>
                  </td>
                  <td>
                    <span class="cell-name"><?= htmlspecialchars($c['kullanici_adi']) ?></span>
                    <span class="cell-sub"><?= htmlspecialchars($c['email']) ?></span>
                  </td>
                  <td class="cell-date"><?= date('d.m.Y H:i',strtotime($c['created_at'])) ?></td>
                  <td>
                    <a href="admin.php?action=contacts&delete_contact=<?= $c['id'] ?>" class="btn-delete"
                       onclick="return confirm('Bu acil kişiyi silmek istediğine emin misin?')">
                      <i class="fas fa-trash-alt"></i> Sil
                    </a>
                  </td>
                </tr>
      ...
      ?>
        <div class="section-header">
          <h1>İhbar Yönetimi</h1>
          <p>Toplam <?= count($reports) ?> ihbar kaydı.</p>
        </div>
        <div class="card">
          <div class="card-head">
            <div class="card-title"><i class="fas fa-triangle-exclamation"></i> Tüm İhbarlar</div>
            <span class="card-count"><?= count($reports) ?> kayıt</span>
          </div>
          <div class="table-wrap">
            <table class="data-table">
              <thead><tr><th>#</th><th>Kullanıcı</th><th>E-posta</th><th>Konum</th><th>Tarih</th><th>İşlem</th></tr></thead>
              <tbody>
              <?php if (count($reports)==0): ?>
                <tr><td colspan="6"><div class="empty-state"><i class="fas fa-check-circle"></i><p>Hiç ihbar bulunmuyor.</p></div></td></tr>
              <?php else: foreach ($reports as $r): ?>
                <tr>
                  ...
                  <td>
                    <a class="map-link" href="https://maps.google.com/?q=<?= $r['lat'] ?>,<?= $r['lng'] ?>" target="_blank">
                  ...
                  <td class="cell-date"><?= date('d.m.Y H:i',strtotime($r['created_at'])) ?></td>
                  <td>
                    <a href="admin.php?action=reports&delete_report=<?= $r['id'] ?>" class="btn-delete"
                       onclick="return confirm('Bu ihbarı silmek istediğine emin misin?')">
                      <i class="fas fa-trash-alt"></i> Sil
                    </a>
                  </td>
                </tr>
      ...
      ?>
        <div class="section-header">
          ...
          <p><?= count($photo_rows) ?> fotoğraf · <?= count($audio_rows) ?> ses kaydı</p>
        </div>

        <div class="card">
          <div class="media-tabs">
            <button class="tab-btn active" onclick="switchTab('photos', this)">
              <i class="fas fa-images" style="margin-right:6px;"></i>Fotoğraflar (<?= count($photo_rows) ?>)
            </button>
            <button class="tab-btn" onclick="switchTab('audios', this)">
              <i class="fas fa-headphones" style="margin-right:6px;"></i>Ses Kayıtları (<?= count($audio_rows) ?>)
            </button>
          </div>

          <div class="tab-panel active" id="tab-photos">
            <?php if (count($photo_rows)==0): ?>
              <div class="empty-state"><i class="fas fa-image"></i><p>Henüz fotoğraf yüklenmemiş.</p></div>
            <?php else: ?>
            <div class="photo-grid">
              <?php foreach ($photo_rows as $photo):
                // file_path DB'de "uploads/dosya.png" şeklinde geliyor.
                // admin.php ile aynı dizinde olduğundan doğrudan kullan.
                // http ile başlıyorsa olduğu gibi bırak.
                $photo_src = (strpos($photo['file_path'], 'http') === 0)
                  ? $photo['file_path']
                  : $photo['file_path']; // relative — tarayıcı admin.php dizininden bulur
              ?>
              <div class="photo-item">
                <img src="<?= htmlspecialchars($photo_src) ?>"
                     alt="Fotoğraf"
                     loading="lazy"
                     onerror="this.parentElement.querySelector('.photo-broken').style.display='flex';this.style.display='none';">
                <div class="photo-broken" style="display:none;height:140px;background:var(--surface-3);align-items:center;justify-content:center;flex-direction:column;gap:6px;color:var(--muted);">
                  <i class="fas fa-image-slash" style="font-size:24px;"></i>
                  <span style="font-size:11px;">Görüntülenemedi</span>
                </div>
                <div class="photo-meta">
                  <div class="pname"><?= htmlspecialchars($photo['ad_soyad']) ?></div>
                  <div class="pdate"><?= date('d.m.Y H:i',strtotime($photo['created_at'])) ?></div>
                  <a href="admin.php?action=media&delete_photo=<?= $photo['id'] ?>" class="btn-delete"
                     onclick="return confirm('Fotoğrafı silmek istediğine emin misin?')">
                    <i class="fas fa-trash-alt"></i> Sil
                  </a>
                </div>
              </div>
          ...
          <div class="tab-panel" id="tab-audios">
            <?php if (count($audio_rows)==0): ?>
              <div class="empty-state"><i class="fas fa-microphone-slash"></i><p>Henüz ses kaydı yüklenmemiş.</p></div>
            <?php else: ?>
            <div class="audio-list-wrap">
              <?php foreach ($audio_rows as $audio):
                $audio_src = (strpos($audio['file_path'], 'http') === 0)
                  ? $audio['file_path']
                  : $audio['file_path'];
              ?>
              <div class="audio-row">
                ...
                <div class="audio-meta">
                  <div class="aname"><?= htmlspecialchars($audio['ad_soyad']) ?></div>
                  <div class="adate"><?= date('d.m.Y H:i',strtotime($audio['created_at'])) ?></div>
                </div>
                <div class="audio-player">
                  <audio controls>
                    <source src="<?= htmlspecialchars($audio_src) ?>" type="audio/webm">
                    Tarayıcınız desteklemiyor.
                  </audio>
                </div>
                <a href="admin.php?action=media&delete_audio=<?= $audio['id'] ?>" class="btn-delete"
                   onclick="return confirm('Ses kaydını silmek istediğine emin misin?')">
      ...
      <?php elseif ($action == 'calls'):
        // Kullanıcı bazında çağrı özeti
        $call_summary = $conn->query("
          SELECT u.id, u.ad_soyad, u.email, u.kan_grubu,
                 COUNT(s.id)                                        AS toplam,
                 SUM(s.type = 'polis' OR s.type = 'police')        AS polis,
                 SUM(s.type = 'acil_durum' OR s.type = 'emergency' OR s.type = 'sos') AS acil,
                 MAX(s.created_at)                                  AS son_cagri
          FROM users u
          JOIN sent_logs s ON s.user_id = u.id
          WHERE u.is_admin = 0
          GROUP BY u.id
          ORDER BY toplam DESC
        ");
        $summary_rows = [];
        if ($call_summary) while ($row = $call_summary->fetch_assoc()) $summary_rows[] = $row;

        // Tüm çağrı detayları (son 100)
        $all_logs = $conn->query("
          SELECT s.id, s.type, s.created_at, u.ad_soyad, u.email
          FROM sent_logs s
        ...
        ");
        $log_rows = [];
        if ($all_logs) while ($row = $all_logs->fetch_assoc()) $log_rows[] = $row;

        // Bugün / bu hafta sayısı
      ...
      ?>
        ...
        <div class="section-header">
          <h1>Polis &amp; Acil Çağrı Geçmişi</h1>
          <p>Kullanıcıların kaç kez ve ne zaman acil çağrı yaptığını buradan takip edebilirsiniz.</p>
        </div>

        <!-- Mini istatistik şerit -->
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:28px;">
          <div class="stat-card" style="--stat-color:#f87171;">
            <div class="stat-row">
              <div class="stat-icon-wrap" style="background:rgba(248,113,113,0.15);color:#f87171;">
                <i class="fas fa-phone-volume"></i>
              </div>
            </div>
            <div class="stat-num"><?= $toplam_calls ?></div>
          ...
          <div class="stat-card" style="--stat-color:#f59e0b;">
            ...
            <div class="stat-num"><?= $bugun_calls ?></div>
            <div class="stat-label">Bugünkü Çağrılar</div>
          </div>
          <div class="stat-card" style="--stat-color:#60a5fa;">
            <div class="stat-row">
              <div class="stat-icon-wrap" style="background:rgba(96,165,250,0.15);color:#60a5fa;">
                <i class="fas fa-calendar-week"></i>
              </div>
            </div>
            <div class="stat-num"><?= $hafta_calls ?></div>
            <div class="stat-label">Son 7 Günde</div>
          </div>
        </div>

        <!-- Kullanıcı bazında özet tablo -->
        <div class="card" style="margin-bottom:20px;">
          <div class="card-head">
            <div class="card-title"><i class="fas fa-users"></i> Kullanıcı Bazlı Çağrı Özeti</div>
            <span class="card-count"><?= count($summary_rows) ?> kullanıcı</span>
          </div>
          <div class="table-wrap">
            <table class="data-table">
              <thead>
              ...
              <?php if (count($summary_rows) == 0): ?>
                <tr><td colspan="7">
                  <div class="empty-state"><i class="fas fa-phone-slash"></i><p>Henüz hiç çağrı kaydı yok.</p></div>
                </td></tr>
              <?php else: foreach ($summary_rows as $row): ?>
                <tr>
                  ...
                  <td>
                    <?php if ($row['kan_grubu']): ?>
                      <span style="background:rgba(248,113,113,0.15);color:#f87171;padding:3px 10px;border-radius:20px;font-size:13px;font-weight:700;">
                        <i class="fas fa-droplet" style="font-size:10px;margin-right:3px;"></i><?= htmlspecialchars($row['kan_grubu']) ?>
                      </span>
                    <?php else: ?>
                      <span style="color:var(--muted);font-size:12px;">—</span>
                    <?php endif; ?>
                  ...
                  <td>
                    <span style="background:rgba(239,68,68,0.12);color:#f87171;padding:4px 12px;border-radius:20px;font-size:14px;font-weight:700;font-family:'Syne',sans-serif;">
                      <?= (int)$row['toplam'] ?>
                    </span>
                  </td>
                  <td>
                    <?php $p = (int)$row['polis']; ?>
                    <span style="background:<?= $p>0?'rgba(59,130,246,0.12)':'var(--surface-3)' ?>;color:<?= $p>0?'#60a5fa':'var(--muted)' ?>;padding:3px 10px;border-radius:20px;font-size:13px;font-weight:600;">
                      <?= $p ?>
                    </span>
                  </td>
                  <td>
                    <?php $a = (int)$row['acil']; ?>
                    <span style="background:<?= $a>0?'rgba(245,158,11,0.12)':'var(--surface-3)' ?>;color:<?= $a>0?'#f59e0b':'var(--muted)' ?>;padding:3px 10px;border-radius:20px;font-size:13px;font-weight:600;">
                      <?= $a ?>
                    </span>
                  </td>
                  <td class="cell-date"><?= $row['son_cagri'] ? date('d.m.Y H:i', strtotime($row['son_cagri'])) : '—' ?></td>
                  <td>
                    <a href="admin.php?action=calls&delete_user_calls=<?= $row['id'] ?>" class="btn-delete"
                       onclick="return confirm('<?= addslashes(htmlspecialchars($row['ad_soyad'])) ?> adlı kullanıcının tüm çağrı kayıtlarını silmek istediğine emin misin?')">
                      <i class="fas fa-trash-alt"></i> Tüm Kayıtlarını Sil
                    </a>
                  </td>
                </tr>
              <?php endforeach; endif; ?>
        ...
        <div class="card">
          <div class="card-head">
            ...
            <div style="display:flex;align-items:center;gap:10px;">
              ...
              <a href="admin.php?action=calls&delete_all_calls=1" class="btn-delete"
                 style="font-size:12px;"
                 onclick="return confirm('Tüm çağrı kayıtlarını silmek istediğine emin misin? Bu işlem geri alınamaz!')">
          ...
          <div class="table-wrap">
            <table class="data-table">
              <thead>
                <tr><th>#</th><th>Kullanıcı</th><th>E-posta</th><th>Çağrı Türü</th><th>Tarih &amp; Saat</th><th>İşlem</th></tr>
              </thead>
              <tbody>
              <?php if (count($log_rows) == 0): ?>
                <tr><td colspan="5"><div class="empty-state"><i class="fas fa-phone-slash"></i><p>Kayıt yok.</p></div></td></tr>
              <?php else: foreach ($log_rows as $lg):
                $t = strtolower($lg['type'] ?? '');
                if ($t === 'polis' || $t === 'police') {
                  $badge_bg  = 'rgba(59,130,246,0.12)'; $badge_c = '#60a5fa';
                  $badge_icon = 'fas fa-shield-halved'; $badge_label = 'Polis';
                } else {
                  $badge_bg  = 'rgba(239,68,68,0.12)'; $badge_c = '#f87171';
                  $badge_icon = 'fas fa-triangle-exclamation'; $badge_label = 'Acil / SOS';
                }
              ?>
                <tr>
                  ...
                  <td>
                    <span style="background:<?= $badge_bg ?>;color:<?= $badge_c ?>;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;display:inline-flex;align-items:center;gap:5px;">
                      <i class="<?= $badge_icon ?>"></i> <?= $badge_label ?>
                    </span>
                  </td>
                  <td class="cell-date"><?= date('d.m.Y H:i', strtotime($lg['created_at'])) ?></td>
                  <td>
                    <a href="admin.php?action=calls&delete_call=<?= $lg['id'] ?>" class="btn-delete"
                       onclick="return confirm('Bu çağrı kaydını silmek istediğine emin misin?')">
                      <i class="fas fa-trash-alt"></i> Sil
                    </a>
                  </td>
                </tr>
